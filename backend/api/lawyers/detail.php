...
function handleUpdateLawyer($db, $lawyer_id, $user_id) {
    ...
    error_log("handleUpdateLawyer - raw payload: " . $rawPayload);
    error_log("handleUpdateLawyer - decoded payload: " . print_r($data, true));

    if(isset($data['verified']) && (!isset($data['name']) && !isset($data['provider_type']) && !isset($data['registration_status']))){
        try{
            $db->beginTransaction();
            $upd = $db->prepare("UPDATE lawyer_profiles SET verified = :verified WHERE lawyer_id = :lawyer_id");
            $verifiedVal = $data['verified'] ? true : false;
            $upd->bindParam(':verified', $verifiedVal, PDO::PARAM_BOOL);
            $upd->bindParam(':lawyer_id', $lawyer_id);
            $upd->execute();
            ...
            $description = $data['update_description'] ?? ($verifiedVal ? 'Profile verified' : 'Profile unverified');
            ...
            $db->commit();
            echo json_encode(["message" => "Verification status updated"]);
            return;
        }catch(PDOException $e){
            $db->rollback();
            http_response_code(500);
